...
func TestPluginBuildServesHomePage(t *testing.T) {
	...
	req := newChineseRequest(http.MethodGet, "/", nil)
...
func TestPluginBuildServesFeedAPIWithETag(t *testing.T) {
	...
	root := t.TempDir()
	publishSignedTopicPost(t, root, "World keep", "aip2p/world", []string{"world"})
...
func TestPluginBuildServesColdStartHomeShell(t *testing.T) {
	...
	site := buildContentSiteAtRoot(t, root)
	req := newChineseRequest(http.MethodGet, "/", nil)
	req.Header.Set("X-Aip2p-Debug-ColdStart", "1")
	rec := httptest.NewRecorder()
	site.Handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, body = %s", rec.Code, rec.Body.String())
...
func TestPluginBuildRendersMarkdownSafelyOnPostPage(t *testing.T) {
	...
	site := buildContentSiteAtRoot(t, root)
	req := newChineseRequest(http.MethodGet, "/posts/"+result.InfoHash, nil)
	rec := httptest.NewRecorder()
	site.Handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, body = %s", rec.Code, rec.Body.String())
...
func TestPluginBuildServesPendingApprovalPage(t *testing.T) {
	...
	root := t.TempDir()
	publishSignedTopicPost(t, root, "World keep", "aip2p/world", []string{"world"})
	publishSignedTopicPost(t, root, "Tech pending", "aip2p/tech", []string{"technology"})
...
func TestPluginBuildPendingApprovalShowsBatchModerationForm(t *testing.T) {
	...
	root := t.TempDir()
	result := publishSignedTopicPost(t, root, "Tech pending", "aip2p/tech", []string{"technology"})
  ...
  "topics": ["world"]
	...
	req := newChineseRequest(http.MethodGet, "/pending-approval?reviewer=reviewer-usa&q=tech", nil)
	rec := httptest.NewRecorder()
	site.Handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, body = %s", rec.Code, rec.Body.String())
	...
	for _, want := range []string{
		...
		`name="redirect" value="/pending-approval?lang=zh-CN&amp;q=tech&amp;reviewer=reviewer-usa"`,
...
func TestPluginBuildBatchModerationApprovePromotesSelectedPosts(t *testing.T) {
	...
	root := t.TempDir()
	first := publishSignedTopicPost(t, root, "Tech pending one", "aip2p/tech", []string{"technology"})
	second := publishSignedTopicPost(t, root, "Tech pending two", "aip2p/tech", []string{"technology"})
	writeTestSigningIdentity(t, root, "moderator-signing")
...
func TestPluginBuildServesModerationReviewersPage(t *testing.T) {
	...
	site := buildContentSiteAtRoot(t, root)
	req := newChineseRequest(http.MethodGet, "/moderation/reviewers", nil)
	rec := httptest.NewRecorder()
	site.Handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, body = %s", rec.Code, rec.Body.String())
...
func TestPluginBuildPendingApprovalShowsSuggestedReviewer(t *testing.T) {
  ...
  "topics": ["world"]
	...
	req := newChineseRequest(http.MethodGet, "/pending-approval", nil)
	rec := httptest.NewRecorder()
	site.Handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, body = %s", rec.Code, rec.Body.String())
...
func TestPluginBuildModerationReviewersCanCreateReviewerIdentity(t *testing.T) {
	...
	identity, err := coreaip2p.LoadAgentIdentity(path)
	if err != nil {
...
func TestPluginBuildModerationReviewersShowsRecentActions(t *testing.T) {
	...
	root := t.TempDir()
	result := publishSignedTopicPost(t, root, "World pending", "aip2p/world", []string{"world"})
	writeRootReviewerIdentity(t, root, "reviewer-root")
...
func TestPluginBuildModerationReviewersCanFilterRecentActionsByReviewer(t *testing.T) {
	...
	root := t.TempDir()
	resultRoot := publishSignedTopicPost(t, root, "World pending root", "aip2p/world", []string{"world"})
	resultUSA := publishSignedTopicPost(t, root, "World pending usa", "aip2p/world", []string{"world"})
	writeRootReviewerIdentity(t, root, "reviewer-root")
	writeDelegatedReviewerIdentity(t, root, "reviewer-usa", []string{"moderation:approve:topic/world"})
  ...
  "topics": ["news"]
	...
	req = newChineseRequest(http.MethodGet, "/moderation/reviewers?reviewer=reviewer-usa", nil)
	rec = httptest.NewRecorder()
	site.Handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, body = %s", rec.Code, rec.Body.String())
...
func TestPluginBuildPendingApprovalConfiguredParentRoute(t *testing.T) {
	...
	root := t.TempDir()
	result := publishHDChildTestPost(t, root, "Signed from hd child")
	parent, err := coreaip2p.RecoverHDIdentity(
...
func TestPluginBuildPendingApprovalAutoRouteBalancesReviewers(t *testing.T) {
	...
	root := t.TempDir()
	publishSignedTopicPost(t, root, "Tech pending A", "aip2p/tech", []string{"technology"})
	publishSignedTopicPost(t, root, "Tech pending B", "aip2p/tech", []string{"technology"})
	publishSignedTopicPost(t, root, "Tech pending C", "aip2p/tech", []string{"technology"})
...
func buildContentSiteAtRoot(t *testing.T, root string) *apphost.Site {
	...
	site, err := Plugin{}.Build(context.Background(), cfg, themeaip2p.Theme{})
	if err != nil {
...
func publishSignedTestPost(t *testing.T, root, body string) coreaip2p.PublishResult {
	t.Helper()

	identity, err := coreaip2p.NewAgentIdentity(
		"agent://aip2p/test-writer",
		"agent://demo/alice",
	...
	store, err := coreaip2p.OpenStore(filepath.Join(root, "store"))
	if err != nil {
		t.Fatalf("OpenStore() error = %v", err)
	}
	result, err := coreaip2p.PublishMessage(store, coreaip2p.MessageInput{
		Kind:     "post",
...
func publishHDChildTestPost(t *testing.T, root, body string) coreaip2p.PublishResult {
	t.Helper()
	...
	child, err := coreaip2p.DeriveChildIdentity(parent, "agent://pc76/openclaw01", timestamp(2026, 3, 19, 12, 57, 40))
	if err != nil {
...
func publishSignedTopicPost(t *testing.T, root, title, channel string, topics []string) coreaip2p.PublishResult {
	t.Helper()
...
func writeTestSigningIdentity(t *testing.T, root, name string) {
	...
	if err := coreaip2p.SaveAgentIdentity(path, identity); err != nil {
		t.Fatalf("SaveAgentIdentity() error = %v", err)
	}
}

func writeRootReviewerIdentity(t *testing.T, root, name string) coreaip2p.AgentIdentity {
	t.Helper()

	identity, err := coreaip2p.RecoverHDIdentity(
		"agent://aip2p/test-admin-root",
		"agent://demo",
		"anchor chicken able drum crush cable negative strong hybrid sister refuse venture spoil rebuild orchard brain jacket gauge summer coconut sibling scissors legend wife",
...
func writeChildReviewerIdentity(t *testing.T, root, name string) coreaip2p.AgentIdentity {
	t.Helper()

	parent := writeRootReviewerIdentity(t, root, "reviewer-root")
	child, err := coreaip2p.DeriveChildIdentity(parent, "agent://demo/"+name, timestamp(2026, 3, 28, 13, 1, 0))
	if err != nil {
		t.Fatalf("DeriveChildIdentity() error = %v", err)
	}
	path := filepath.Join(root, "config", "identities", name+".json")
	...
	if err := coreaip2p.SaveAgentIdentity(path, child); err != nil {
		t.Fatalf("SaveAgentIdentity() error = %v", err)
	}
	return child
}
...
func writeDelegatedReviewerIdentity(t *testing.T, root, name string, scopes []string) {
	...
	if err := coreaip2p.SaveAgentIdentity(identityPath, child); err != nil {
		t.Fatalf("SaveAgentIdentity() error = %v", err)
	}

	seed, err := coreaip2p.MnemonicToSeed(parent.Mnemonic)
	if err != nil {
		t.Fatalf("MnemonicToSeed() error = %v", err)
	}
	path, err := coreaip2p.PathFromURI(parent.Author)
	if err != nil {
		t.Fatalf("PathFromURI() error = %v", err)
	}
	_, parentPrivateKey, _, err := coreaip2p.DeriveHDKey(seed, path)
	if err != nil {
