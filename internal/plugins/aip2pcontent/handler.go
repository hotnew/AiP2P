...
func ajaxFragmentRequest(r *http.Request) bool {
	return strings.TrimSpace(r.Header.Get("X-Aip2p-Ajax")) == "1"
}

func setAjaxVariantHeaders(w http.ResponseWriter) {
	w.Header().Add("Vary", "X-Aip2p-Ajax")
}
...
func forceColdStartForTests(r *http.Request) bool {
	...
	return strings.TrimSpace(r.Header.Get("X-Aip2p-Debug-ColdStart")) == "1"
}
...
func handleHome(app *newsplugin.App, w http.ResponseWriter, r *http.Request) {
	...
	if showNetworkWarn {
		...
		http.SetCookie(w, &http.Cookie{
			Name:     "aip2p_network_warning_seen",
			Value:    "1",
			Path:     "/",
			MaxAge:   180 * 24 * 60 * 60,
			HttpOnly: true,
...
func handleModerationReviewerUpdate(app *newsplugin.App, w http.ResponseWriter, r *http.Request) {
	...
	rootIdentity, err := aip2p.LoadAgentIdentity(rootPath)
	if err != nil {
	...
	case "create":
		...
		childIdentity, err := aip2p.DeriveChildIdentity(rootIdentity, childAuthor, time.Now().UTC())
		if err != nil {
			http.Redirect(w, r, "/moderation/reviewers?moderation_error=identity", http.StatusSeeOther)
			return
		}
		if err := aip2p.SaveAgentIdentity(identityPath, childIdentity); err != nil {
			http.Redirect(w, r, "/moderation/reviewers?moderation_error=save", http.StatusSeeOther)
			return
		}
...
func handleModeration(app *newsplugin.App, w http.ResponseWriter, r *http.Request) {
	...
	identity, err := aip2p.LoadAgentIdentity(identityPath)
	if err != nil {
...
func buildModerationDecision(app *newsplugin.App, post newsplugin.Post, action string, identity aip2p.AgentIdentity, identityLabel, reviewer string) (newsplugin.ModerationDecision, error) {
...
func handleAPIBundle(app *newsplugin.App, w http.ResponseWriter, r *http.Request) {
	...
	payload, err := aip2p.BundleTarPayload(store, infoHash, 0)
	if err != nil {
...
func handlePostVote(app *newsplugin.App, w http.ResponseWriter, r *http.Request) {
	...
	store, err := aip2p.OpenStore(app.StoreRoot())
	if err != nil {
	...
	_, err = aip2p.PublishMessage(store, aip2p.MessageInput{
		Kind:     "reaction",
		Author:   strings.TrimSpace(identity.Author),
		Channel:  "aip2p/reactions",
		Body:     body,
		Identity: &identity,
...
func moderationIdentityAuthorized(identity aip2p.AgentIdentity, scopes []string, store newsplugin.DelegationStore, now time.Time) bool {
...
func moderationIdentityRank(identity aip2p.AgentIdentity, post newsplugin.Post, action string, store newsplugin.DelegationStore, now time.Time) int {
	if strings.TrimSpace(identity.ParentPublicKey) == "" {
		return 1
	}
	rank := 0
...
func moderationIdentityScopes(identity aip2p.AgentIdentity, store newsplugin.DelegationStore, now time.Time) []string {
	if strings.TrimSpace(identity.ParentPublicKey) == "" {
...
func moderationSuggestionReason(post newsplugin.Post, identity aip2p.AgentIdentity, app *newsplugin.App) string {
	if strings.TrimSpace(identity.ParentPublicKey) == "" {
		return "root"
	}
	store, err := newsplugin.LoadDelegationStore(
...
func shouldShowNetworkWarning(r *http.Request) bool {
	...
	cookie, err := r.Cookie("aip2p_network_warning_seen")
	if err != nil {
		return true
	}
