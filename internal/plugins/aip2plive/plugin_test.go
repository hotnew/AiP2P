...
import (
	...
	core "aip2p/internal/aip2p"
	"aip2p/internal/aip2p/live"
	"aip2p/internal/apphost"
	aip2ptheme "aip2p/internal/themes/aip2p"
)
...
func TestLiveAnnouncementWatcherNetPath(t *testing.T) {
	...
	prevEnv, hadEnv := os.LookupEnv("AIP2P_DISABLE_LIVE_ANNOUNCEMENT_WATCHER")
	if err := os.Unsetenv("AIP2P_DISABLE_LIVE_ANNOUNCEMENT_WATCHER"); err != nil {
		t.Fatalf("Unsetenv error = %v", err)
	}
	defer func() {
		if hadEnv {
			_ = os.Setenv("AIP2P_DISABLE_LIVE_ANNOUNCEMENT_WATCHER", prevEnv)
			return
		}
		_ = os.Unsetenv("AIP2P_DISABLE_LIVE_ANNOUNCEMENT_WATCHER")
	}()

	root := t.TempDir()
	netPath := filepath.Join(root, "aip2p_net.inf")
	...
	liveNetPath := filepath.Join(root, "aip2p_live_net.inf")
...
func TestNewHandlerServesLiveStatusAPI(t *testing.T) {
	...
	senderNetPath := filepath.Join(root, "config", "aip2p_live_sender_net.inf")
...
func TestPluginBuildServesLiveStatusPage(t *testing.T) {
	...
	req := newChineseRequest(http.MethodGet, "/live/status/public-live-time", nil)
	rec := httptest.NewRecorder()
	site.Handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, body = %s", rec.Code, rec.Body.String())
...
func TestPluginBuildLimitsVisibleLiveEventsButCanShowAll(t *testing.T) {
	...
	req := newChineseRequest(http.MethodGet, "/live/room-window", nil)
	rec := httptest.NewRecorder()
	site.Handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, body = %s", rec.Code, rec.Body.String())
...
func TestPluginBuildServesPublicLiveRoomDespiteBlockedOrigin(t *testing.T) {
	...
	req := newChineseRequest(http.MethodGet, "/live/public/new-agents", nil)
	rec := httptest.NewRecorder()
	site.Handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, body = %s", rec.Code, rec.Body.String())
...
func TestPluginBuildServesLivePublicModerationPage(t *testing.T) {
	...
	req := newChineseRequest(http.MethodGet, "/live/public/moderation", nil)
	rec := httptest.NewRecorder()
	site.Handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, body = %s", rec.Code, rec.Body.String())
...
func TestPluginBuildServesLiveRoomDetails(t *testing.T) {
	...
	req := newChineseRequest(http.MethodGet, "/live/"+room.RoomID, nil)
	rec := httptest.NewRecorder()
	site.Handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, body = %s", rec.Code, rec.Body.String())
...
func TestPluginBuildServesLiveRoomHistoryAfterPrune(t *testing.T) {
	...
	req = newChineseRequest(http.MethodGet, "/live/history/"+room.RoomID, nil)
	rec = httptest.NewRecorder()
	site.Handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
...
func TestPluginBuildCreatesManualArchiveAndServesArchiveLiveRoutes(t *testing.T) {
	...
	indexReq := newChineseRequest(http.MethodGet, "/archive/live", nil)
	...
	historyListReq := newChineseRequest(http.MethodGet, "/archive/live/"+room.RoomID, nil)
...
func buildLiveSite(t *testing.T) (*apphost.Site, string) {
	...
	cfg := apphost.Config{
		...
		Version:          "test",
		Plugin:           "aip2p-live",
		Plugins:          []string{"aip2p-content", "aip2p-live"},
	}
	site, err := Plugin{}.Build(context.Background(), cfg, aip2ptheme.Theme{})
	if err != nil {
