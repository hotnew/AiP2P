...
const projectSyncBinaryName = "aip2p-syncd"
...
# Plaintext file loaded by --net ~/.aip2p/aip2p_net.inf
#
# Supported keys:
...
# aip2p should treat libp2p as the primary control plane for discovery and subscriptions.
...
libp2p_rendezvous=aip2p/global
libp2p_rendezvous=aip2p/world
...
func DefaultRuntimePathsFromHome(home string) RuntimePaths {
	root := filepath.Join(strings.TrimSpace(home), ".aip2p")
...
func ensureNetworkIDFile(path, networkID string) error {
	...
	content := "# Stable 256-bit aip2p network namespace.\nnetwork_id=" + networkID + "\n"
