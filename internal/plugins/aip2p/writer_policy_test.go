...
func TestApplyWriterPolicyOnlyKeepsReadWriteOrigins(t *testing.T) {
	...
	postAllowed := Bundle{
		...
		Message: Message{
			...
			Extensions: map[string]any{"project": "aip2p", "topics": []any{"all", "world"}},
		},
	}
	postReadOnly := Bundle{
	...
	index := buildIndex([]Bundle{postAllowed, postReadOnly, replyReadOnly}, "aip2p")
	policy := WriterPolicy{
	...
	filtered := ApplyWriterPolicy(index, "aip2p", policy)
...
func TestApplyWriterPolicyWhitelistAcceptsOnlyExplicitWriters(t *testing.T) {
	...
	index := buildIndex([]Bundle{postAllowed, postOther}, "aip2p")
	policy := WriterPolicy{
...
func TestApplyWriterPolicyParentAndChildrenTrustsChildAuthorWhenRootWhitelisted(t *testing.T) {
	...
	postOther := Bundle{
		...
		Message: Message{
			...
			Extensions: map[string]any{"project": "aip2p"},
		},
	}

	index := buildIndex([]Bundle{postChild, postOther}, "aip2p")
	policy := WriterPolicy{
		SyncMode:          WriterSyncModeWhitelist,
...
func TestApplyWriterPolicyBlacklistOverridesParentAndChildrenTrust(t *testing.T) {
	...
	index := buildIndex([]Bundle{postChild}, "aip2p")
	policy := WriterPolicy{
		SyncMode:          WriterSyncModeWhitelist,
		TrustMode:         WriterTrustModeParentAndChildren,
		DefaultCapability: WriterCapabilityReadWrite,
...
func TestApplyWriterPolicyAllModeKeepsSignedReadOnlyWritersUnlessBlocked(t *testing.T) {
	...
	index := buildIndex([]Bundle{postReadOnly, postBlocked}, "aip2p")
	policy := WriterPolicy{
...
func TestApplyWriterPolicyAllModeRejectsUnsignedByDefault(t *testing.T) {
	...
	index := buildIndex([]Bundle{unsignedPost, noKeyPost}, "aip2p")
	policy := WriterPolicy{
		SyncMode:          WriterSyncModeAll,
		AllowUnsigned:     false,
		DefaultCapability: WriterCapabilityReadWrite,
...
func TestApplyWriterPolicyDelegationUsesParentCapability(t *testing.T) {
	...
	index := buildIndex([]Bundle{post}, "aip2p")
	policy := WriterPolicy{
	...
	filtered := ApplyWriterPolicyWithDelegations(index, "aip2p", policy, store)
	if len(filtered.Posts) != 1 {
		t.Fatalf("posts len = %d, want 1", len(filtered.Posts))
	}
