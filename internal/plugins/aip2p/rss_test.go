...
func TestWriteTopicRSS(t *testing.T) {
	...
	if err := WriteTopicRSS(rr, req, "aip2p Public", "world", posts); err != nil {
		t.Fatalf("write rss: %v", err)
	}
	body := rr.Body.String()
	for _, needle := range []string{"<rss version=\"2.0\"", "<title>aip2p Public Topic: world</title>", "<link>https://ai.jie.news/topics/world</link>", "<title>World Story</title>", "<guid isPermaLink=\"false\">abc123</guid>"} {
