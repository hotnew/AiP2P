...
func SignWriterDelegation(delegation WriterDelegation, identity coreaip2p.AgentIdentity) (WriterDelegation, error) {
...
func SignWriterRevocation(revocation WriterRevocation, identity coreaip2p.AgentIdentity) (WriterRevocation, error) {
	privateKey, err := signingPrivateKey(identity)
	if err != nil {
		return WriterRevocation{}, err
	}
...
func signingPrivateKey(identity coreaip2p.AgentIdentity) (ed25519.PrivateKey, error) {
	...
	seed, err := coreaip2p.MnemonicToSeed(identity.Mnemonic)
	if err != nil {
		return nil, err
	}
	_, privateKey, _, err := coreaip2p.DeriveHDKey(seed, identity.DerivationPath)
	if err != nil {
		return nil, err
	}
