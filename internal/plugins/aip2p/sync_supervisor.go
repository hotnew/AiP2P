...
func resolveManagedSyncBinary(cfg ManagedSyncConfig) (string, error) {
	...
	if exe, err := os.Executable(); err == nil {
		candidates = append(candidates,
			...
			filepath.Join(filepath.Dir(exe), "aip2p"+platformExecutableSuffix()),
			filepath.Join(filepath.Dir(exe), "aip2pd"+platformExecutableSuffix()),
		)
	}
	...
	return "", errors.New("managed sync binary not found; build aip2p-syncd into ~/.aip2p/bin, install aip2p into PATH, or pass --sync-binary")
}
