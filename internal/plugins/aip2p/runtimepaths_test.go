...
func TestDefaultRuntimePathsFromHome(t *testing.T) {
	...
	if paths.Root != "/tmp/example-home/.aip2p" {
		t.Fatalf("root = %q", paths.Root)
	}
	if paths.StoreRoot != "/tmp/example-home/.aip2p/aip2p/.aip2p" {
		t.Fatalf("store = %q", paths.StoreRoot)
	}
	if paths.IdentitiesRoot != "/tmp/example-home/.aip2p/identities" {
		t.Fatalf("identities = %q", paths.IdentitiesRoot)
	}
	if paths.DelegationsRoot != "/tmp/example-home/.aip2p/delegations" {
		t.Fatalf("delegations = %q", paths.DelegationsRoot)
	}
	if paths.RevocationsRoot != "/tmp/example-home/.aip2p/revocations" {
		t.Fatalf("revocations = %q", paths.RevocationsRoot)
	}
	if paths.ArchiveRoot != "/tmp/example-home/.aip2p/archive" {
		t.Fatalf("archive = %q", paths.ArchiveRoot)
	}
	if paths.RulesPath != "/tmp/example-home/.aip2p/subscriptions.json" {
		t.Fatalf("rules = %q", paths.RulesPath)
	}
	if paths.WriterPolicyPath != "/tmp/example-home/.aip2p/writer_policy.json" {
		t.Fatalf("writer policy = %q", paths.WriterPolicyPath)
	}
	if paths.WriterWhitelistPath != "/tmp/example-home/.aip2p/WriterWhitelist.inf" {
		t.Fatalf("writer whitelist = %q", paths.WriterWhitelistPath)
	}
	if paths.WriterBlacklistPath != "/tmp/example-home/.aip2p/WriterBlacklist.inf" {
		t.Fatalf("writer blacklist = %q", paths.WriterBlacklistPath)
	}
	if paths.NetPath != "/tmp/example-home/.aip2p/aip2p_net.inf" {
		t.Fatalf("net = %q", paths.NetPath)
	}
	if paths.NetworkIDPath != "/tmp/example-home/.aip2p/network_id.inf" {
...
func TestEnsureRuntimeLayoutCreatesDefaultConfigFiles(t *testing.T) {
	...
	if !strings.Contains(netText, "# redis_enabled=false") || !strings.Contains(netText, "# redis_key_prefix=aip2p-") || !strings.Contains(netText, "# redis_max_announcements=70000") {
	...
	if !strings.Contains(string(idData), "network_id="+latestOrgNetworkID) {
		t.Fatalf("missing aip2p network id in network_id.inf: %q", string(idData))
	}
}
...
func TestEnsureRuntimeLayoutAppendsLatestNetworkIDToExistingNetConfig(t *testing.T) {
	...
	if err := os.WriteFile(netPath, []byte("libp2p_rendezvous=aip2p/global\n"), 0o644); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
	if err := ensureRuntimeLayout(store, archive, rules, writerPolicy, netPath); err != nil {
		t.Fatalf("ensureRuntimeLayout() error = %v", err)
	...
	if strings.Contains(string(data), "network_id=") {
		t.Fatalf("network_id should no longer be appended to aip2p_net.inf: %q", string(data))
	}
	idData, err := os.ReadFile(networkIDPath)
	if err != nil {
		t.Fatalf("ReadFile(networkIDPath) error = %v", err)
