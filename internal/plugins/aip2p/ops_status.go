...
func redisNodeStatus(cfg NetworkBootstrapConfig) (NodeStatusEntry, NodeStatusCard) {
	...
	if cfg.Redis.Enabled {
		...
		if err := coreaip2p.ProbeRedis(cfg.Redis, 1200*time.Millisecond); err != nil {
			...
			if summary, err := coreaip2p.ReadRedisSyncSummary(cfg.Redis, 1200*time.Millisecond); err == nil {
				detail = fmt.Sprintf(
...
func (a *App) buildNodeStatus(index Index) NodeStatus {
	...
	discoveryTone := "warn"
	discoveryDetail := "Add aip2p_net.inf to declare bootstrap peers and routers."
	if netErr != nil {
		discoveryValue = "config error"
		discoveryTone = "bad"
		discoveryDetail = "aip2p_net.inf could not be read."
	...
	httpFallbackDetail := "aip2p now uses libp2p direct transfer with HTTP fallback for bundle delivery."
	...
	networkIDTone := "warn"
	networkIDDetail := "Add a stable 256-bit network_id in network_id.inf so same-name projects do not share the same aip2p discovery space."
	if netErr != nil {
	...
	case netErr != nil:
		...
		summaryTone = "bad"
		summaryDetail = "aip2p_net.inf exists but could not be parsed."
	case len(netCfg.LibP2PBootstrap) > 0:
		summary = "bootstrap ready"
		summaryTone = "good"
		summaryDetail = "libp2p discovery is configured. aip2p Public is still in UI/index mode until the sync daemon is running."
	}
	return NodeStatus{
		Summary:       summary,
...
func buildLiveNodeStatus(index Index, storeState, storeTone string, torrentCount int, netCfg NetworkBootstrapConfig, syncStatus SyncRuntimeStatus, listenAddr string) NodeStatus {
	...
	if !netCfg.Exists {
		...
		discoveryDetail = "Sync daemon is running, but aip2p_net.inf is not present on this node."
	}
...
func (a *App) lanPeerHealth() ([]LANPeerHealthStatus, []LANPeerHealthStatus, error) {
	cfg, err := coreaip2p.LoadNetworkBootstrapConfig(a.netPath)
	if err != nil {
		return nil, nil, err
	}
	lanPeers, lanBTPeers, err := coreaip2p.ReadLANPeerHealthStatus(cfg)
	if err != nil {
		return nil, nil, err
	}
...
func (a *App) knownGoodLibP2PPeers() ([]KnownGoodLibP2PPeerStatus, error) {
	...
	items, err := coreaip2p.ReadKnownGoodLibP2PPeerStatus(cfg)
	if err != nil {
		return nil, err
	}
...
func mapLANPeerHealthStatus(values []coreaip2p.LANPeerHealthStatus) []LANPeerHealthStatus {
...
func lanPeerHealthReason(value coreaip2p.LANPeerHealthStatus) string {
