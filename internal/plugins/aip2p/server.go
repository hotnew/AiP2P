...
func newApp(storeRoot, project, version, archiveRoot, rulesPath, writerPath, netPath string, theme apphost.WebTheme, options AppOptions) (*App, error) {
	...
	funcs := template.FuncMap{
		...
		"formatFlexibleTime": func(value any) string {
			switch typed := value.(type) {
			case time.Time:
				if typed.IsZero() {
					return "none yet"
				}
				return defaultDisplayTime(typed).Format("2006-01-02 15:04 MST")
			case string:
				parsed, err := time.Parse(time.RFC3339, strings.TrimSpace(typed))
				if err != nil || parsed.IsZero() {
					return strings.TrimSpace(typed)
				}
				return defaultDisplayTime(parsed).Format("2006-01-02 15:04 MST")
			default:
				return ""
			}
		},
