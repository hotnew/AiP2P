...
func (a *App) HistoryListPayload(cursor string, pageSize int) (HistoryManifestAPIResponse, error) {
	...
	for _, bundle := range bundles[start:end] {
		ref := aip2p.CanonicalSyncRef(strings.ToLower(strings.TrimSpace(bundle.InfoHash)), strings.TrimSpace(bundle.Message.Title))
		ref = aip2p.WithSourcePeerHintForSyncRef(ref, sourceHost)
		ref = aip2p.WithLibP2PPeerHintForSyncRef(ref, localPeerID)
