...
func handleTeamSync(app *newsplugin.App, store *teamcore.Store, teamID string, w http.ResponseWriter, r *http.Request) {
	...
	conflicts, err := coreaip2p.LoadTeamSyncConflicts(app.StoreRoot(), teamID, coreaip2p.TeamSyncConflictFilter{Limit: 10})
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func handleAPITeamSync(app *newsplugin.App, store *teamcore.Store, teamID string, w http.ResponseWriter, r *http.Request) {
	...
	conflicts, err := coreaip2p.LoadTeamSyncConflicts(app.StoreRoot(), teamID, coreaip2p.TeamSyncConflictFilter{Limit: limit})
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func loadTeamSyncRuntimeStatus(storeRoot string) (coreaip2p.SyncRuntimeStatus, error) {
	path := filepath.Join(strings.TrimSpace(storeRoot), "sync", "status.json")
	data, err := os.ReadFile(path)
	if errors.Is(err, os.ErrNotExist) {
		return coreaip2p.SyncRuntimeStatus{}, nil
	}
	if err != nil {
		return coreaip2p.SyncRuntimeStatus{}, err
	}
	var status coreaip2p.SyncRuntimeStatus
...
func resolveTeamSyncConflict(app *newsplugin.App, store *teamcore.Store, teamID, conflictKey, remoteAddr, actorAgentID, action string) (coreaip2p.TeamSyncConflictRecord, error) {
	request := &http.Request{RemoteAddr: remoteAddr}
	if !teamRequestTrusted(request) {
		return coreaip2p.TeamSyncConflictRecord{}, errors.New("team sync conflict resolution is limited to local or LAN requests")
	}
	actorAgentID = strings.TrimSpace(actorAgentID)
	action = strings.TrimSpace(action)
	if err := requireTeamConflictAction(store, teamID, actorAgentID); err != nil {
		return coreaip2p.TeamSyncConflictRecord{}, err
	}
	record, err := coreaip2p.ResolveTeamSyncConflict(app.StoreRoot(), teamID, conflictKey, action, actorAgentID)
	if err != nil {
		return coreaip2p.TeamSyncConflictRecord{}, err
	}
...
func buildTeamSyncConflictViews(records []coreaip2p.TeamSyncConflictRecord) []teamSyncConflictView {
...
func buildOpenTeamSyncConflictViews(records []coreaip2p.TeamSyncConflictRecord) []teamSyncConflictView {
...
func buildResolvedTeamSyncConflictViews(records []coreaip2p.TeamSyncConflictRecord) []teamSyncConflictView {
	views := buildTeamSyncConflictViews(records)
	out := make([]teamSyncConflictView, 0, len(views))
	for _, view := range views {
		if strings.TrimSpace(view.Record.Resolution) != "" {
...
func conflictActionPermissions(record coreaip2p.TeamSyncConflictRecord, allowAcceptRemote bool) (bool, bool) {
	if strings.TrimSpace(record.Resolution) != "" {
		return false, false
	}
...
func buildTeamSyncConflictActions(record coreaip2p.TeamSyncConflictRecord, allowKeepLocal, allowAcceptRemote bool, suggestedAction string) []teamSyncConflictActionView {
	if strings.TrimSpace(record.Resolution) != "" {
		return nil
	}
...
func describeTeamSyncConflictSubject(record coreaip2p.TeamSyncConflictRecord) string {
...
func classifyTeamSyncConflict(record coreaip2p.TeamSyncConflictRecord) string {
	switch strings.TrimSpace(record.Reason) {
	case "remote_newer":
		return "safe-remote"
	case "local_newer":
...
func describeTeamSyncConflictSeverity(record coreaip2p.TeamSyncConflictRecord) string {
	switch strings.TrimSpace(record.Reason) {
	case "signature_rejected", "policy_rejected":
		return "blocked"
	case "same_version_diverged":
...
func describeTeamSyncConflictConsequence(record coreaip2p.TeamSyncConflictRecord) string {
	switch strings.TrimSpace(record.Reason) {
	case "remote_newer":
...
func buildTeamSyncStatusGroups(status coreaip2p.SyncTeamSyncStatus, webhook teamcore.WebhookDeliveryStatus) []teamSyncStatusGroup {
...
func describeTeamSyncConflict(record coreaip2p.TeamSyncConflictRecord, allowAcceptRemote bool) (string, string, string) {
...
func teamSyncHealthLevel(status coreaip2p.SyncTeamSyncStatus, webhook teamcore.WebhookDeliveryStatus) string {
...
func teamSyncHealthTitle(status coreaip2p.SyncTeamSyncStatus, webhook teamcore.WebhookDeliveryStatus) string {
...
func teamSyncHealthHint(status coreaip2p.SyncTeamSyncStatus, webhook teamcore.WebhookDeliveryStatus) string {
	switch teamSyncHealthLevel(status, webhook) {
	case "disabled":
...
func countUnresolvedTeamConflicts(conflicts []coreaip2p.TeamSyncConflictRecord) int {
	count := 0
...
func countResolvedTeamConflicts(storeRoot, teamID string) int {
	conflicts, err := coreaip2p.LoadTeamSyncConflicts(storeRoot, teamID, coreaip2p.TeamSyncConflictFilter{
