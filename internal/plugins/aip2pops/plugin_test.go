...
func TestPluginBuildServesNetworkPage(t *testing.T) {
	...
	site := buildOpsSite(t)
	req := newChineseRequest(http.MethodGet, "/network", nil)
...
func TestPluginBuildServesBootstrapRedisSummary(t *testing.T) {
	...
	defer mini.Close()
	netText := "network_mode=lan\nredis_enabled=true\nredis_addr=" + mini.Addr() + "\nredis_db=2\nredis_key_prefix=aip2p-test-\n"
	if err := os.WriteFile(netPath, []byte(netText), 0o644); err != nil {
		t.Fatalf("WriteFile(netPath) error = %v", err)
	}
	rdb := redis.NewClient(&redis.Options{Addr: mini.Addr(), DB: 2})
	defer rdb.Close()
	if err := rdb.Set(context.Background(), "aip2p-test-sync:ann:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", `{"infohash":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","channel":"news","topics":["world"]}`, time.Hour).Err(); err != nil {
		t.Fatalf("Set(sync ann) error = %v", err)
	}
	if err := rdb.ZAdd(context.Background(), "aip2p-test-sync:channel:news", redis.Z{Score: 1711933200, Member: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}).Err(); err != nil {
		t.Fatalf("ZAdd(channel) error = %v", err)
	}
	if err := rdb.ZAdd(context.Background(), "aip2p-test-sync:topic:world", redis.Z{Score: 1711933200, Member: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}).Err(); err != nil {
		t.Fatalf("ZAdd(topic) error = %v", err)
	}
	if err := rdb.RPush(context.Background(), "aip2p-test-sync:queue:refs:realtime", "aip2p-sync://bundle/aaa?dn=one").Err(); err != nil {
		t.Fatalf("RPush(realtime) error = %v", err)
	}
	if err := rdb.RPush(context.Background(), "aip2p-test-sync:queue:refs:history", "aip2p-sync://bundle/bbb?dn=two").Err(); err != nil {
	...
	if payload.Redis.Addr != mini.Addr() || payload.Redis.Prefix != "aip2p-test-" || payload.Redis.DB != 2 {
...
func TestPluginBuildServesCreditPage(t *testing.T) {
	...
	root := t.TempDir()
	seedCreditProof(t, root, "agent://alice/credit/online", coreaip2p.AlignToWindow(time.Now().UTC()).Add(-10*time.Minute))

	site := buildOpsSiteAtRoot(t, root)
	req := newChineseRequest(http.MethodGet, "/credit", nil)
	rec := httptest.NewRecorder()
	site.Handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, body = %s", rec.Code, rec.Body.String())
...
func TestPluginBuildServesCreditAuthorView(t *testing.T) {
	...
	root := t.TempDir()
	seedCreditProof(t, root, "agent://alice/credit/online", coreaip2p.AlignToWindow(time.Now().UTC()).Add(-20*time.Minute))
	seedCreditProof(t, root, "agent://alice/credit/online", coreaip2p.AlignToWindow(time.Now().UTC()).Add(-10*time.Minute))

	site := buildOpsSiteAtRoot(t, root)
	req := newChineseRequest(http.MethodGet, "/credit?author=agent://alice/credit/online&start=2026-03-01&end=2026-03-31", nil)
	rec := httptest.NewRecorder()
	site.Handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, body = %s", rec.Code, rec.Body.String())
...
func TestPluginBuildServesCreditPagePagination(t *testing.T) {
	...
	root := t.TempDir()
	day := coreaip2p.AlignToWindow(time.Now().UTC()).Add(-10 * time.Minute)
	seedCreditProof(t, root, "agent://alice/credit/online", day)
	seedCreditProof(t, root, "agent://bob/credit/online", day)

	site := buildOpsSiteAtRoot(t, root)
	req := newChineseRequest(http.MethodGet, "/credit?date="+day.Format("2006-01-02")+"&page_size=1&page=2", nil)
	rec := httptest.NewRecorder()
	site.Handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, body = %s", rec.Code, rec.Body.String())
...
func TestPluginBuildServesCreditBalanceAPI(t *testing.T) {
	...
	var payload struct {
		...
		Balance coreaip2p.CreditBalance `json:"balance"`
	}
	if err := json.Unmarshal(rec.Body.Bytes(), &payload); err != nil {
		t.Fatalf("json.Unmarshal() error = %v", err)
	}
...
func TestPluginBuildServesCreditProofsAPI(t *testing.T) {
	...
	root := t.TempDir()
	windowStart := coreaip2p.AlignToWindow(time.Now().UTC()).Add(-10 * time.Minute)
	...
	var payload struct {
		...
		Proofs     []coreaip2p.OnlineProof    `json:"proofs"`
...
func TestPluginBuildServesCreditStatsAPI(t *testing.T) {
	...
	root := t.TempDir()
	seedCreditProof(t, root, "agent://alice/credit/online", coreaip2p.AlignToWindow(time.Now().UTC()).Add(-20*time.Minute))
	seedCreditProof(t, root, "agent://bob/credit/online", coreaip2p.AlignToWindow(time.Now().UTC()).Add(-10*time.Minute))
	...
	var payload struct {
		...
		Balances     []coreaip2p.CreditBalance         `json:"balances"`
		Issues       []string                          `json:"issues"`
		Daily        []coreaip2p.CreditDailyStat       `json:"daily"`
		WitnessRoles []coreaip2p.CreditWitnessRoleStat `json:"witness_roles"`
	}
	if err := json.Unmarshal(rec.Body.Bytes(), &payload); err != nil {
		t.Fatalf("json.Unmarshal() error = %v", err)
	}
...
func seedCreditProof(t *testing.T, root, author string, windowStart time.Time) coreaip2p.OnlineProof {
	t.Helper()

	store, err := coreaip2p.OpenCreditStore(filepath.Join(root, "store"))
	if err != nil {
		t.Fatalf("OpenCreditStore() error = %v", err)
	}
	node, err := coreaip2p.NewAgentIdentity("agent://news/node-01", author, time.Now().UTC())
	if err != nil {
		t.Fatalf("NewAgentIdentity(node) error = %v", err)
	}
	witness, err := coreaip2p.NewAgentIdentity("agent://news/witness-01", "agent://witness/credit/online", time.Now().UTC())
	if err != nil {
		t.Fatalf("NewAgentIdentity(witness) error = %v", err)
	}
	proof, err := coreaip2p.NewOnlineProof(node, windowStart, []string{"abc123"}, "aip2p-mainnet")
	if err != nil {
		t.Fatalf("NewOnlineProof() error = %v", err)
	}
	if err := coreaip2p.SignProof(proof, node); err != nil {
		t.Fatalf("SignProof() error = %v", err)
	}
	if err := coreaip2p.AddWitnessSignature(proof, witness, "dht_neighbor"); err != nil {
