...
func startSession(ctx context.Context, opts SessionOptions) (_ *session, err error) {
	identity, err := aip2p.LoadAgentIdentity(strings.TrimSpace(opts.IdentityFile))
	if err != nil {
		return nil, err
	}
	...
	if strings.TrimSpace(opts.NetPath) != "" {
		if err := aip2p.EnsureDefaultNetworkBootstrapConfig(opts.NetPath); err != nil {
			return nil, err
		}
	}
	netCfg, err := aip2p.LoadNetworkBootstrapConfig(opts.NetPath)
	if err != nil {
		return nil, err
	}
	...
	info := RoomInfo{
		...
		Channel:         firstNonEmpty(strings.TrimSpace(opts.Channel), "aip2p/live"),
	}
...
func startTransport(ctx context.Context, cfg aip2p.NetworkBootstrapConfig) (host.Host, *kaddht.IpfsDHT, mdns.Service, *routingdisc.RoutingDiscovery, *pubsub.PubSub, error) {
	var options []libp2p.Option
	resolvedLANPeers, _ := resolveLiveLANBootstrapPeers(ctx, cfg)
	resolvedPublicPeers, _ := aip2p.ResolveExplicitBootstrapPeers(ctx, cfg.PublicPeers, cfg.NetworkID, "public_peer")
	resolvedRelayPeers, _ := aip2p.ResolveExplicitBootstrapPeers(ctx, cfg.RelayPeers, cfg.NetworkID, "relay_peer")
	if factory := aip2p.BuildLibP2PAddrsFactory(cfg); factory != nil {
	...
	if len(cfg.LibP2PListen) > 0 {
		resolved, err := aip2p.ResolveLibP2PListenAddrs(cfg.LibP2PListen)
		if err != nil {
	...
	dhtOptions := []kaddht.Option{kaddht.Mode(aip2p.DHTModeForConfig(cfg))}
	...
	serviceName := "_aip2p-live._udp"
	if strings.TrimSpace(cfg.NetworkID) != "" && len(cfg.NetworkID) >= 12 {
		serviceName = "_aip2p-live-" + cfg.NetworkID[:12] + "._udp"
	}
