...
func publishArchiveNoticeDetached(netPath string, identity aip2p.AgentIdentity, info RoomInfo, result ArchiveResult) error {
	netPath = strings.TrimSpace(netPath)
	if netPath == "" {
		return fmt.Errorf("net path is required for detached archive notice")
	}
	if err := aip2p.EnsureDefaultNetworkBootstrapConfig(netPath); err != nil {
		return err
	}
	netCfg, err := aip2p.LoadNetworkBootstrapConfig(netPath)
	if err != nil {
		return err
	}
