...
func TestSignAndVerifyMessage(t *testing.T) {
	identity, err := aip2p.NewAgentIdentity("agent://pc75", "agent://pc75/openclaw01", time.Now().UTC())
	if err != nil {
...
func TestSignAndVerifyChildMessageWithDelegation(t *testing.T) {
	rootIdentity, err := aip2p.RecoverHDIdentity(
		"pc75",
		"agent://pc75",
	...
	childIdentity, err := aip2p.DeriveChildIdentity(rootIdentity, "agent://pc75/live-alpha", time.Date(2026, 3, 30, 8, 0, 0, 0, time.UTC))
	if err != nil {
...
func TestReadEventsIgnoresPartialTrailingJSONLine(t *testing.T) {
	...
	if _, err := file.WriteString("{\"protocol\":\"aip2p-live/0.1\""); err != nil {
		_ = file.Close()
...
func TestAnnouncementWatcherHandleArchiveNotice(t *testing.T) {
	...
	result := ArchiveResult{
		...
		Published: aip2p.PublishResult{
			...
			Ref:      "aip2p-sync://bundle/338f294e18ac2fe39c0a5201845bc6e4d7cc33c0?dn=live-archive",
		},
	}
...
func TestAnnouncementWatcherJoinDoesNotOverrideExistingRoomOwner(t *testing.T) {
	...
	identity, err := aip2p.NewAgentIdentity("agent://pc75", "agent://pc75/live-bravo", time.Now().UTC())
	if err != nil {
		t.Fatalf("NewAgentIdentity error = %v", err)
	}
