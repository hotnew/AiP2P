...
type ArchiveResult struct {
	...
	Published  aip2p.PublishResult `json:"published"`
...
func Archive(opts ArchiveOptions) (ArchiveResult, error) {
	var redisCfg aip2p.RedisConfig
	...
	store, err := aip2p.OpenStore(opts.StoreRoot)
	if err != nil {
		return ArchiveResult{}, err
	}
	...
	if channel == "" {
		channel = firstNonEmpty(info.Channel, "aip2p/live")
	}
