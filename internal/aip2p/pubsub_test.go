...
func TestSubscribedAnnouncementTopics(t *testing.T) {
	...
	if !containsString(topics, "aip2p/announce/"+networkID+"/topic/world") {
		t.Fatalf("missing topic subscription: %v", topics)
	}
	if !containsString(topics, "aip2p/announce/"+networkID+"/tag/breaking") {
		t.Fatalf("missing tag subscription: %v", topics)
	}
	if !containsString(topics, "aip2p/announce/"+networkID+"/channel/aip2p%2Fnews") {
		t.Fatalf("missing discovery feed subscription: %v", topics)
	}
	if !containsString(topics, "aip2p/announce/"+networkID+"/topic/futures") {
...
func TestDiscoveryNamespacesIncludeConfiguredFeedsAndTopics(t *testing.T) {
	...
	networkID := "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"
	namespaces := discoveryNamespaces(networkID, []string{"aip2p/global"}, SyncSubscriptions{
		DiscoveryFeeds:  []string{"global", "news"},
		DiscoveryTopics: []string{"国际"},
	})
	if !containsString(namespaces, "aip2p/discovery/"+networkID+"/aip2p%2Fglobal") {
		t.Fatalf("missing configured namespace: %v", namespaces)
	}
	if !containsString(namespaces, "aip2p/discovery/"+networkID+"/feed%2Fglobal") {
		t.Fatalf("missing global discovery feed: %v", namespaces)
	}
	if !containsString(namespaces, "aip2p/discovery/"+networkID+"/feed%2Fnews") {
		t.Fatalf("missing news discovery feed: %v", namespaces)
	}
	if !containsString(namespaces, "aip2p/discovery/"+networkID+"/topic%2Fworld") {
...
func TestTeamSyncDiscoveryNamespaceUsesNetworkScopedTeamPath(t *testing.T) {
	...
	want := "aip2p/discovery/" + networkID + "/team%2Farchive-demo%2Fsync"
	if got != want {
...
func TestSubscribedAnnouncementTopicsCanonicalizesDiscoveryFeeds(t *testing.T) {
	...
	networkID := "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"
	topics := subscribedAnnouncementTopics(networkID, SyncSubscriptions{
		DiscoveryFeeds: []string{"aip2p/news", "NEWS", "all", "新手"},
	})
	if !containsString(topics, "aip2p/announce/"+networkID+"/global") {
		t.Fatalf("missing global subscription: %v", topics)
	}
	if !containsString(topics, "aip2p/announce/"+networkID+"/channel/aip2p%2Fnews") {
		t.Fatalf("missing news channel subscription: %v", topics)
	}
	if !containsString(topics, "aip2p/announce/"+networkID+"/channel/aip2p%2Fnew-agents") {
