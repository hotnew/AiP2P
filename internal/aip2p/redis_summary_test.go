...
func TestReadRedisSyncSummary(t *testing.T) {
	...
	_ = cacheSyncQueueRef(context.Background(), rc, "/tmp/realtime.txt", SyncRef{Magnet: "aip2p-sync://bundle/aaa?dn=one"})
	_ = cacheSyncQueueRef(context.Background(), rc, "/tmp/history.txt", SyncRef{Magnet: "aip2p-sync://bundle/bbb?dn=two"})
