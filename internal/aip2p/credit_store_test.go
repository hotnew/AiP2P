...
func TestCreditStoreBalanceAndQueries(t *testing.T) {
	...
	proof1 := mustCreditProof(t, "agent://alice/credit/online", AlignToWindow(time.Now().UTC()).Add(-20*time.Minute), "aip2p-mainnet")
	proof2 := mustCreditProof(t, "agent://alice/credit/online", AlignToWindow(time.Now().UTC()).Add(-10*time.Minute), "aip2p-mainnet")
...
func TestCreditStoreCleanOldProofs(t *testing.T) {
	...
	oldProof := mustCreditProof(t, "agent://alice/credit/online", AlignToWindow(time.Now().UTC()).AddDate(0, 0, -120), "aip2p-mainnet")
...
func TestCreditStoreArchiveProofsKeepsQueriesWorking(t *testing.T) {
	...
	proof := mustCreditProof(t, "agent://alice/credit/online", oldWindow, "aip2p-mainnet")
...
func TestCreditStoreStats(t *testing.T) {
	...
	if err := store.SaveProof(mustCreditProofWithWitnessRole(t, "agent://alice/credit/online", day1, "aip2p-mainnet", "dht_neighbor")); err != nil {
		t.Fatalf("SaveProof(day1 alice) error = %v", err)
	}
	if err := store.SaveProof(mustCreditProofWithWitnessRole(t, "agent://bob/credit/online", day1.Add(10*time.Minute), "aip2p-mainnet", "random_check")); err != nil {
		t.Fatalf("SaveProof(day1 bob) error = %v", err)
	}
	if err := store.SaveProof(mustCreditProofWithWitnessRole(t, "agent://alice/credit/online", day2, "aip2p-mainnet", "dht_neighbor")); err != nil {
...
func TestCreditStoreCacheInvalidatesAfterSaveProof(t *testing.T) {
	...
	proof1 := mustCreditProof(t, "agent://alice/credit/online", window.Add(-20*time.Minute), "aip2p-mainnet")
	proof2 := mustCreditProof(t, "agent://alice/credit/online", window.Add(-10*time.Minute), "aip2p-mainnet")
	if err := store.SaveProof(proof1); err != nil {
		t.Fatalf("SaveProof(proof1) error = %v", err)
	}
...
func TestCreditStoreGetBalanceUsesAuthorScopedView(t *testing.T) {
	...
	window := AlignToWindow(time.Now().UTC())
	if err := store.SaveProof(mustCreditProof(t, "agent://alice/credit/online", window.Add(-20*time.Minute), "aip2p-mainnet")); err != nil {
		t.Fatalf("SaveProof(alice) error = %v", err)
	}
	if err := store.SaveProof(mustCreditProof(t, "agent://bob/credit/online", window.Add(-10*time.Minute), "aip2p-mainnet")); err != nil {
