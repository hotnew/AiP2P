...
func TestPluginFilesIncludeBasePlugin(t *testing.T) {
	...
	if got := paths["aip2p.plugin.json"]; got == "" || !strings.Contains(got, `"base_plugin": "aip2p-content"`) {
...
func TestAppFilesUseLocalPluginPack(t *testing.T) {
	...
	if got := paths["plugins/sample-app-plugin/aip2p.plugin.json"]; got == "" || !strings.Contains(got, `"base_plugin": "aip2p-content"`) {
