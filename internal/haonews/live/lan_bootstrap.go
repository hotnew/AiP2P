...
func resolveLiveLANBootstrapPeers(ctx context.Context, cfg haonews.NetworkBootstrapConfig) ([]string, error) {
	...
	fallback, err := haonews.ResolveLANBootstrapPeers(ctx, cfg)
	if err == nil {
		return fallback, nil
	}
	if len(errs) == 0 {
