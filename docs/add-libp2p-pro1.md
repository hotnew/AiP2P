# aip2p libp2p 直传优化落地方案 (add-libp2p-pro1)

## 目标

基于 [libp2p-pro1.md](/Users/haoniu/sh18/aip2p2/libp2p-pro1.md) 的方向，为 aip2p 当前 `internal/aip2p` 同步链路补上“小 bundle 优先走 libp2p 直传，失败再回退 BT”的能力，同时尽量复用现有 pubsub、sync、store、status 结构，避免把队列格式和历史回流路径一次性改得过重。

## 当前代码对应关系

- libp2p host/runtime: `aip2p/internal/aip2p/libp2p.go`
- announcement/pubsub: `aip2p/internal/aip2p/pubsub.go`
- sync 主循环与 `syncRef`: `aip2p/internal/aip2p/sync.go`
- store / torrent 路径: `aip2p/internal/aip2p/store.go`
- 状态输出: `aip2p/internal/aip2p/status.go`
- 网络配置: `aip2p/internal/aip2p/network.go`
