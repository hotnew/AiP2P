...
- 基础提案：`libp2p-pro1.md`
- 输出要求：
  - 先形成 repo 对应的方案文档 `docs/add-libp2p-pro1.md`
  - 再按该思路整改代码
  - 把整个过程整理到本文件
...
- `aip2p/internal/aip2p/libp2p.go`
- `aip2p/internal/aip2p/pubsub.go`
- `aip2p/internal/aip2p/sync.go`
- `aip2p/internal/aip2p/network.go`
- `aip2p/internal/aip2p/status.go`
- `aip2p/internal/aip2p/store.go`
...
- `aip2p/internal/aip2p/transfer.go`

实现了：

- `/aip2p/bundle-transfer/1.0` stream protocol
- provider handler
- tar 打包/解包
- SHA-256 校验
- 安全解包
...
最开始按文档中的 `internal/aip2p` 去搜，当前仓库并没有这个目录。后面改成对 `aip2p/internal/aip2p/*` 真实结构落地。
...
- `aip2p/internal/aip2p/status.go`
- `aip2p/internal/aip2p/sync.go`
- `aip2p/internal/aip2p/sync_test.go`
- `aip2p/internal/aip2p/transfer.go`
- `docs/add-libp2p-pro1.md`

同时工作区里原本就还有与本任务无关的未提交改动：

- `aip2p/internal/aip2p/live/archive.go`
- `aip2p/internal/aip2p/live/protocol_test.go`
