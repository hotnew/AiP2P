...
这份文件只记录当前 `aip2p` 仓库仍然有效的近期架构调整、同步修复和部署结果。
...
- 回归测试已补：
  ...
  - `aip2plive` 测试已加 watcher 禁用开关，避免并行测试清理时的 flaky cleanup
...
## 2026-03-27 15:03 CST - 同步引用从 magnet 迁移到 aip2p-sync
...
- 新同步引用格式：
  - `aip2p-sync://bundle/<infohash>?peer=...`
- `SyncAnnouncement` 新增 `ref`
- `/api/history/list` 开始返回 `ref`
- 队列新写入统一使用 `aip2p-sync://...`
- 旧 `magnet` 继续兼容读取
...
- 确认 `.75(shared)` 真正的 managed worker 是：
  - `~/.aip2p/bin/aip2p-syncd`
- 确认公网机真实运行目录是：
  - `/var/lib/aip2p/.aip2p`
- 将 `aip2p` 和 `aip2p-syncd` 统一为同一版二进制
...
- 公网机真实运行目录里的队列已整理成：
  - `realtime.txt` 只剩少量新帖
  - `history.txt` 使用 `aip2p-sync://bundle/<infohash>?peer=...`
...
- `/api/feed`、`/api/topics`、`/api/topics/<topic>` 冷启动时先返回：
  - `starting=true`
  - 空列表
- `aip2plive` 的 `AnnouncementWatcher` 改成后台启动
...
- `go test ./internal/aip2p -run 'TestSubscribedAnnouncementTopics|TestDiscoveryNamespacesIncludeConfiguredFeedsAndTopics|TestMatchesAnnouncement|TestMatchesHistoryAnnouncementUsesHistorySelectors'`
- `go test ./internal/plugins/aip2p -run 'TestLoadSubscriptionRulesNormalizesDiscoverySelectors|TestApplySubscriptionRules'`
...
- `go test ./internal/aip2p -run 'TestSubscribedAnnouncementTopics|TestDiscoveryNamespacesIncludeConfiguredFeedsAndTopics|TestMatchesAnnouncement|TestMatchesHistoryAnnouncementUsesHistorySelectors'`
- `go test ./internal/plugins/aip2p -run 'TestLoadSubscriptionRulesNormalizesDiscoverySelectors|TestApplySubscriptionRules'`
- `go test ./internal/plugins/aip2pops`
...
- `go test ./internal/aip2p -run 'TestSubscribedAnnouncementTopics|TestDiscoveryNamespacesIncludeConfiguredFeedsAndTopics|TestSubscribedAnnouncementTopicsCanonicalizesDiscoveryFeeds'`
- `go test ./internal/plugins/aip2p -run 'TestLoadSubscriptionRulesNormalizesDiscoverySelectors|TestApplySubscriptionRules'`
...
- `go test ./internal/aip2p -run 'TestSubscribedAnnouncementTopics|TestDiscoveryNamespacesIncludeConfiguredFeedsAndTopics|TestMatchesAnnouncement|TestMatchesHistoryAnnouncementUsesHistorySelectors|TestSyncSubscriptionsNormalizeCanonicalizesTopicAliases'`
- `go test ./internal/plugins/aip2p -run 'TestLoadSubscriptionRulesNormalizesDiscoverySelectors|TestLoadSubscriptionRulesNormalizesTopicAliases|TestApplySubscriptionRules|TestFilterPostsCanonicalizesTopicAliases|TestBuildIndexCanonicalizesTopicAliases'`
...
- `go test ./internal/aip2p -run 'TestSubscribedAnnouncementTopics|TestDiscoveryNamespacesIncludeConfiguredFeedsAndTopics|TestMatchesAnnouncement|TestMatchesHistoryAnnouncementUsesHistorySelectors|TestSyncSubscriptionsNormalizeCanonicalizesTopicAliases'`
- `go test ./internal/plugins/aip2p -run 'TestLoadSubscriptionRulesNormalizesDiscoverySelectors|TestLoadSubscriptionRulesNormalizesTopicAliases|TestLoadSubscriptionRulesAppliesConfiguredTopicAliasesAndWhitelist|TestApplySubscriptionRules|TestFilterPostsCanonicalizesTopicAliases|TestBuildIndexCanonicalizesTopicAliases'`
...
- `go test ./internal/aip2p -run 'TestSubscribedAnnouncementTopics|TestDiscoveryNamespacesIncludeConfiguredFeedsAndTopics|TestMatchesAnnouncement|TestMatchesHistoryAnnouncementUsesHistorySelectors|TestSyncSubscriptionsNormalizeCanonicalizesTopicAliases'`
- `go test ./internal/plugins/aip2p ./internal/plugins/aip2pops`
...
- 这次不是单一“网络坏了”，而是三层问题叠在一起：
  - `.76` 上一度同时存在两套 `sync` 托管：
    - `aip2p serve` 默认 `managed` 拉起的 worker
    - 额外手工加的 `launchd com.aip2p.sync`
...
- `internal/aip2p/lanpeer_test.go`
...
- 给 `.76` 下发新版 `aip2p`
- 清掉 `.76` 的：
  - `~/.aip2p/known_good_libp2p_peers.json`
- 对远端二进制做 ad-hoc `codesign`
- 删除额外手工加的：
  - `~/Library/LaunchAgents/com.aip2p.sync.plist`
- 回到单一托管模式：
  - 只保留 `aip2p serve` 的 `managed sync`
...
- `go test ./internal/aip2p -run 'TestSubscribedAnnouncementTopicsCanonicalizesDiscoveryFeeds'`
- `go test ./internal/plugins/aip2p -run 'TestLoadSubscriptionRulesNormalizesDiscoverySelectors'`
...
- `internal/plugins/aip2p/types.go`
  - `Post` 新增：
    - `CommentCount`
    - `Upvotes`
    - `Downvotes`
  ...
  - 新增：
    - `TabOption`
- `internal/plugins/aip2p/index.go`
  - 构建索引时统计：
    - `upvotes`
    - `downvotes`
    - `comment_count`
  ...
  - `sortPosts()` 新增：
    - `hot`
- `internal/plugins/aip2p/runtime_content.go`
  - 新增 `BuildTabOptions()`
  - API 输出增加：
    - `comment_count`
    - `upvotes`
  ...
  - `pageURL()/encodeOptions()/APIOptions()` 接通：
    - `tab`
- `internal/plugins/aip2pcontent/handler.go`
  - topic 页面增加：
    - `New`
    - `Hot`
    切换
...
- `go test ./internal/plugins/aip2p -run 'TestFilterPostsSupportsHotTab|TestBuildIndexComputesVoteBreakdownAndHotScore|TestFilterPostsSupportsWindow|TestBuildIndexCanonicalizesTopicAliases'`
- `go test ./cmd/aip2p ./internal/plugins/aip2p ./internal/plugins/aip2pcontent ./internal/plugins/aip2pops`
...
- `internal/plugins/aip2p/types.go`
  ...
  - `FeedOptions` 新增：
    - `PendingApproval`
- `internal/aip2p/subscriptions.go`
  ...
  - 增加默认值与规范化：
    ...
    - `approval`
    - `pending-approval`
- `internal/plugins/aip2p/subscriptions.go`
  ...
  - 主页面统计仍按“可见帖子”重算：
    ...
    - `TopicStats`
    - `SourceStats`
- `internal/plugins/aip2p/server.go`
  - 修正索引流水线
...
- `internal/plugins/aip2p/runtime_content.go`
  ...
  - `APIOptions()` 增加：
    - `approval=pending`
- `internal/plugins/aip2p/runtimepaths.go`
...
- `go test ./internal/plugins/aip2p ./internal/plugins/aip2pcontent`
- 新增回归测试：
...
- `internal/plugins/aip2p/moderation.go`
  - 新增本地审核记录存储：
...
- `internal/plugins/aip2pcontent/handler.go`
  ...
  - 当前 scope 规则：
    ...
    - `moderation:reject:*`
    - `moderation:route:*`
- `internal/plugins/aip2p/runtime_governance_exports.go`
...
- `go test ./internal/plugins/aip2p ./internal/plugins/aip2pcontent ./internal/aip2p`
- 新增回归测试：
...
- `go test ./internal/plugins/aip2pcontent ./internal/plugins/aip2p ./internal/aip2p`
- 新增回归测试：
...
- `internal/plugins/aip2p/delegation.go`
  - 新增：
...
- `go test ./internal/plugins/aip2plive ./internal/aip2p/live`
- `go build ./cmd/aip2p`
- `.75`
  - `/live/public` -> `200`
...
- `go test ./internal/plugins/aip2plive ./internal/aip2p/live ./internal/plugins/aip2p ./internal/aip2p`
