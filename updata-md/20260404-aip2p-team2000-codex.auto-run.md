...
Context
- 源文档：
  - `/Users/haoniu/sh18/aip2p2/aip2p/20260404-aip2p-team2000.md`
- 参考模板：
...
- 当前已知基线：
  ...
  - `.75 / .74` 已作为当前双节点运行基线
  - 现有正式运维文档已经落库：
    - `/Users/haoniu/sh18/aip2p2/aip2p/docs/node-upgrade-75-74.md`
    - `/Users/haoniu/sh18/aip2p2/aip2p/docs/runtime-75-74-baseline.md`
    - `/Users/haoniu/sh18/aip2p2/aip2p/docs/runtime-75-74-validation.md`
- 本轮执行约束：
...
- 必读文件：
  ...
  - `/Users/haoniu/sh18/aip2p2/aip2p/internal/plugins/aip2pteam/handler_webhook.go`
  - `/Users/haoniu/sh18/aip2p2/aip2p/internal/plugins/aip2pteam/a2a_bridge.go`
...
- 按改动补跑：
  - webhook 相关：
    - `go test ./internal/aip2p/team -run 'TestStoreWebhook|Test.*Delivery|Test.*Replay' -count=1`
  - sync/conflict 相关：
  ...
  - A2A 相关：
    - `go test ./internal/plugins/aip2pteam -run 'TestPluginBuildServesA2ABridge' -count=1`
  - SSE 相关：
    - `go test ./internal/plugins/aip2pteam -run 'TestPluginBuildStreamsTeamEvents' -count=1`
- 若做节点复核：
  - `.75` / `.74` 统一看：
    - `/api/teams`
    - `/teams/{teamID}`
