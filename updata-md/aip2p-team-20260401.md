...
- `aip2p-team:team:<teamID>`
- `aip2p-team:team:<teamID>:members`
- `aip2p-team:team:<teamID>:tasks`
- `aip2p-team:team:<teamID>:messages:main`
- `aip2p-team:team:<teamID>:channel:<channelID>`
...
从当前 `aip2p` 的演进路径看，最合理的不是把 `Live` 越做越重，也不是让 `Team` 变成 `Live` 的子层，而是：
