...
> 审阅日期：2026-04-04
> 审阅范围：`internal/aip2p/team/` 包 + `internal/plugins/aip2pteam/` 插件 + `internal/aip2p/team_sync.go` + `internal/aip2p/pubsub.go`
...
3. 确保 `go build ./...` 编译通过
4. 运行 `go test ./internal/aip2p/team/...` 确保测试通过
...
type teamPageData struct {
    ...
    RecentConflicts    []coreaip2p.TeamSyncConflictRecord
...
type teamPageData struct {
    ...
    History    []teamcore.ChangeEvent
    Conflicts  []coreaip2p.TeamSyncConflictRecord
...
| 文件路径 | 行数 | 主要职责 |
|---------|------|---------|
| `internal/aip2p/team/store.go` | 3,462 | 核心数据存储层 |
| `internal/aip2p/team/sync.go` | 928 | 同步消息类型与冲突检测 |
| `internal/aip2p/team/task_state.go` | 115 | 任务状态机 |
| `internal/aip2p/team/agent_card.go` | 248 | Agent 能力注册 |
| `internal/aip2p/team/message_signature.go` | 163 | 消息签名验证 |
| `internal/aip2p/team_sync.go` | 1,946 | PubSub 同步运行时 |
| `internal/aip2p/team_sync_test.go` | 1,065 | 同步测试 |
| `internal/aip2p/pubsub.go` | 1,037 | PubSub 基础设施 |
| `internal/aip2p/sync.go` | 1,964 | 通用同步编排 |
| `internal/plugins/aip2pteam/handler.go` | 1,300+ | HTTP Handler |
| `internal/plugins/aip2pteam/plugin.go` | 124 | 插件入口 |
| `internal/plugins/aip2pteam/types.go` | 201 | 页面数据结构 |
| `internal/plugins/aip2pteam/a2a_bridge.go` | 277 | A2A 协议桥接 |
