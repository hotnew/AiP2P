...
func runPublish(args []string) error {
	...
	if resolvedReplyInfoHash == "" && strings.TrimSpace(*replyRef) != "" {
		ref, err := aip2p.ParseSyncRef(strings.TrimSpace(*replyRef))
		if err != nil {
...
func runLive(args []string) error {
	if len(args) == 0 {
		return errors.New("usage: aip2p live <host|join|list|archive|task-update> [flags]")
	}
	switch args[0] {
	case "host":
...
func runLiveHost(args []string) error {
	...
	channel := fs.String("channel", "aip2p/live", "archive channel hint")
...
func runLiveArchive(args []string) error {
	...
	channel := fs.String("channel", "aip2p/live", "archive channel")
	if err := fs.Parse(args); err != nil {
		return err
	}
	if strings.TrimSpace(*identityFile) == "" {
...
func runIdentityInit(args []string) error {
	...
	out := fs.String("out", "", "identity file output path; defaults to ~/.aip2p/identities/<sanitized-agent-id>.json")
...
func runIdentityCreateHD(args []string) error {
	...
	out := fs.String("out", "", "identity file output path; defaults to ~/.aip2p/identities/<sanitized-author>.json")
	force := fs.Bool("force", false, "overwrite output file if it exists")
	if err := fs.Parse(args); err != nil {
		return err
	}
...
func runIdentityList(args []string) error {
	...
	fs.SetOutput(os.Stderr)
	dir := fs.String("dir", "", "identity directory; defaults to ~/.aip2p/identities")
...
func runCreditBalance(args []string) error {
	...
	storeRoot := fs.String("store", ".aip2p", "credit store root directory")
	if err := fs.Parse(args); err != nil {
		return err
	}
...
func runIdentityRegistryAdd(args []string) error {
	...
	fs.SetOutput(os.Stderr)
	registryPath := fs.String("registry", "", "registry file path; defaults to ~/.aip2p/identity_registry.json")
...
func resolveInfoHashFromLegacyRef(raw string) (string, error) {
	ref, err := aip2p.ParseSyncRef(strings.TrimSpace(raw))
	if err != nil {
		return "", err
	}
...
func defaultIdentityDir(explicitDir string) (string, error) {
	...
	return filepath.Join(home, ".aip2p", "identities"), nil
}
...
func defaultIdentityRegistryPath(explicitPath string) (string, error) {
	...
	return filepath.Join(home, ".aip2p", "identity_registry.json"), nil
}
...
func usageError() error {
	return errors.New("usage: aip2p <identity|credit|live|publish|verify|show|sync|serve|plugins|themes|apps|create> [flags]")
}
