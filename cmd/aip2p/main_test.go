...
func TestNormalizeDeprecatedSyncArgsRewritesMagnetFlag(t *testing.T) {
	...
	args, rewrote, err := normalizeDeprecatedSyncArgs([]string{
		"--magnet", "aip2p-sync://bundle/93a71a010a59022c8670e06e2c92fa279f98d974?dn=test",
		"--once",
	})
	if err != nil {
	...
	got := strings.Join(args, " ")
	if !strings.Contains(got, "--ref aip2p-sync://bundle/93a71a010a59022c8670e06e2c92fa279f98d974?dn=test") {
		t.Fatalf("rewritten args = %q", got)
	}
...
func TestRunPublishReplyRefStoresReplyInfoHashOnly(t *testing.T) {
	...
	if err := run([]string{
		...
		"--title", "Reply post",
		"--body", "reply body",
		"--reply-ref", "aip2p-sync://bundle/93a71a010a59022c8670e06e2c92fa279f98d974?dn=test",
	}); err != nil {
...
func TestDefaultIdentityOutputPathUsesRuntimeIdentityDirectory(t *testing.T) {
	...
	want := filepath.Join(home, ".aip2p", "identities", "agent-news-publisher-01.json")
	if got != want {
...
func TestRunPublishWritesSignedMessage(t *testing.T) {
	...
	if got := msg.Extensions["project"]; got != "aip2p" {
		t.Fatalf("extensions.project = %#v, want aip2p", got)
	}
}
...
func mustMainCreditProof(t *testing.T, author string, windowStart time.Time) aip2p.OnlineProof {
	...
	proof, err := aip2p.NewOnlineProof(node, windowStart, []string{"abc123"}, "aip2p-mainnet")
	if err != nil {
