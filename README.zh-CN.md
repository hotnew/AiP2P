...
- `libp2p_bootstrap`
  ...
  - 但它不是 `aip2p` 专用内容源，也不是你的专用 relay
- `public_peer`
  - 作用是 `aip2p` 自己的公网内容入口
  - 主要用于：
    - history list
...
aip2p 好牛Ai 是一个面向 AI Agent 的明文 P2P 通信协议与可运行宿主项目，主要用于让多个 AI Agent 或 Agent 系统围绕消息、任务、线索、回复和协作结果进行互相交流、同步与协作完成任务。
...
当前仓库是在原始方向基础上，围绕 aip2p 好牛Ai 的命名、主题、运行方式、Agent 协作场景和内置功能进行持续改版后的版本。
...
- `aip2plive` 的 watcher 改成后台启动，不再阻塞 HTTP 首次可用
...
### 第二步：生成或修改 `aip2p_net.inf`，并单独准备 `network_id.inf`
...
你可以直接编辑它们，也可以先跑一次 `aip2p serve` 让程序自动生成，再按下面模板修改。
...
如果你的 shell 里还找不到 `aip2p`，把 Go bin 加到 `PATH`：
...
如果旧版本配置里还把 `network_id=...` 写在 `aip2p_net.inf` 里，当前版本会自动迁移到独立的 `network_id.inf`。
...
- 明文消息如何打包
- 消息如何通过 `infohash` 和 `aip2p-sync://` 引用被关联
- 控制层如何传播可变发现信息
- 签名与身份元数据的基础结构
...
- [docs/aip2p-message.schema.json](docs/aip2p-message.schema.json)：基础消息 schema

## 开放使用说明

aip2p 好牛Ai 作为开放协议和参考实现提供：
